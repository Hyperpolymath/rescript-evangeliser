...
import { Narrative, Pattern, PatternCategory } from './types';

/**
 * Generate an encouraging narrative for a pattern
 */
export function generateNarrative(pattern: Pattern): Narrative {
  // Use pattern's built-in narrative if available
  if (pattern.narrative) {
    return pattern.narrative;
  }

  // Generate dynamic narrative based on category
  return generateCategoryNarrative(pattern.category, pattern.name);
}

/**
 * Generate narrative based on pattern category
 */
function generateCategoryNarrative(category: PatternCategory, patternName: string): Narrative {
  const templates = NARRATIVE_TEMPLATES[category] || NARRATIVE_TEMPLATES['default'];

  return {
    celebrate: pickRandom(templates.celebrate),
    minimize: pickRandom(templates.minimize),
    better: pickRandom(templates.better),
    safety: pickRandom(templates.safety),
...
/**
 * Create a personalized narrative with user's name and pattern details
 */
export function personalizeNarrative(
  ...
  return {
    celebrate: prefix + narrative.celebrate,
    minimize: narrative.minimize,
 ...
 */
export function formatNarrative(narrative: Narrative, format: 'plain' | 'markdown' | 'html' = 'markdown'): string {
  if (format === 'plain') {
    return `
${narrative.celebrate}
 ...
 */
export function generateSuccessMessage(patternName: string, difficulty: string): string {
  const messages = [
 ...
 */
export function generateHint(pattern: Pattern): string {
  const hints = [
    `💡 Tip: ${pattern.bestPractices[0] || 'Practice makes perfect!'}`,
    `🎯 Remember: ${pattern.learningObjectives[0] || 'Focus on understanding the concept.'}`,
    `⚠️ Watch out: ${pattern.commonMistakes[0] || 'Take your time and read the examples.'}`,
